...
def main(args):
    ...
    df1.to_csv("E:\\李雷\\矿石训练最终程序\\矿石图像训练\\ConvNeXt_zhu\\acc.csv", index=False)  # 路径可以根据需要更改

    df2 = pd.DataFrame(columns=['train_loss_list', 'val_loss_list'])  # 列名
    df2.to_csv("E:\\李雷\\矿石训练最终程序\\矿石图像训练\\ConvNeXt_zhu\\loss.csv", index=False)  # 路径可以根据需要更改
    # 将数据保存在一维列表
    # 保存准确率数据
    list1 = [ train_acc_list, val_acc_list]
    # 由于DataFrame是Pandas库中的一种数据结构，它类似excel，是一种二维表，所以需要将list以二维列表的形式转化为DataFrame
    data1 = pd.DataFrame([list1])
    data1.to_csv('E:\\李雷\\矿石训练最终程序\\矿石图像训练\\ConvNeXt_zhu\\acc.csv', mode='a', header=False,
    ...
    data2.to_csv('E:\\李雷\\矿石训练最终程序\\矿石图像训练\\ConvNeXt_zhu\\loss.csv', mode='a', header=False,
                 index=False)  # mode设为a,就可以向csv文件追加数据了
