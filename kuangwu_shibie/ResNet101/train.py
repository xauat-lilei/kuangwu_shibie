...
def main():
    ...
    print("using {} images for training, {} images for validation.".format(train_num,
                                                                           val_num))
    #训练网络
    net = resnet101(num_classes=26, include_top=True)
    net.to(device)
    ...
    df1.to_csv("E:\\李雷\\矿石训练最终程序\\矿石图像训练\\ResNet101\\acc.csv", index=False)  # 路径可以根据需要更改

    df2 = pd.DataFrame(columns=['time_str', 'epoch', 'train_loss', 'val_loss'])  # 列名
    df2.to_csv("E:\\李雷\\矿石训练最终程序\\矿石图像训练\\ResNet101\\loss.csv", index=False)  # 路径可以根据需要更改
    # 将数据保存在一维列表
    # 保存准确率数据
    list1 = [time_str, epoch_list, train_acc_list, val_accurate_list]
    # 由于DataFrame是Pandas库中的一种数据结构，它类似excel，是一种二维表，所以需要将list以二维列表的形式转化为DataFrame
    data1 = pd.DataFrame([list1])
    data1.to_csv('E:\\李雷\\矿石训练最终程序\\矿石图像训练\\ResNet101\\acc.csv', mode='a', header=False,
    ...
    data2.to_csv('E:\\李雷\\矿石训练最终程序\\矿石图像训练\\ResNet101\\loss.csv', mode='a', header=False,
                 index=False)  # mode设为a,就可以向csv文件追加数据了
