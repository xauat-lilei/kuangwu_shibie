...
def main():
    ...
    df1.to_csv("E:\\李雷\\矿石图像训练\\GogleNet\\acc.csv", index=False)  # 路径可以根据需要更改

    df2 = pd.DataFrame(columns=['time_str', 'epoch', 'train_loss', 'val_loss'])  # 列名
    df2.to_csv("E:\\李雷\\矿石图像训练\\GogleNet\\loss.csv", index=False)  # 路径可以根据需要更改
    # 将数据保存在一维列表
    # 保存准确率数据
    list1 = [time_str, epochs, train_acc_list, val_accurate_list]
    # 由于DataFrame是Pandas库中的一种数据结构，它类似excel，是一种二维表，所以需要将list以二维列表的形式转化为DataFrame
    data1 = pd.DataFrame([list1])
    data1.to_csv('E:\\李雷\\矿石图像训练\\GogleNet\\acc.csv', mode='a', header=False, index=False)  # mode设为a,就可以向csv文件追加数据了

    # 保存LOSS数据
    list2 = [time_str, epochs, train_loss_list, val_loss_list]
    # 由于DataFrame是Pandas库中的一种数据结构，它类似excel，是一种二维表，所以需要将list以二维列表的形式转化为DataFrame
    data2 = pd.DataFrame([list2])
    data2.to_csv('E:\\李雷\\矿石图像训练\\GogleNet\\loss.csv', mode='a', header=False, index=False)  # mode设为a,就可以向csv文件追加数据了
